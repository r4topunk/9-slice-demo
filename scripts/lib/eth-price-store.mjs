...
const COINGECKO_HISTORY_URL = (dateDdMmYyyy) =>
...
const toDdMmYyyy = (dateIsoYyyyMmDd) => {
...
const parseCurrentPrice = (payload) => {
...
const parseCoinbaseSpotPrice = (payload) => {
...
const parseHistoricalPrice = (payload) => {
...
export const getGlobalEthUsdPrice = async (ttlMs = 120_000) => {
  const now = Date.now()
  ...
  currentInflight = (async () => {
    ...
    const spotPayload = await spotResponse.json()
...
export const getHistoricalEthUsdPrice = async (dateIsoYyyyMmDd) => {
